...
void sendMavlinkMessages() {
  ...
  int compid = MAV_COMP_ID_SYSTEM_CONTROL;  //50; // Component/Subsystem ID, 1-255
...
void sendHeartBeat() {
  ...
  int compid = MAV_COMP_ID_SYSTEM_CONTROL;  //120;
...
void SendIMUData() {
  ...
  int sysid = 1;
  //< The component sending the message.
  int compid = MAV_COMP_ID_IMU;  //200;
  ...
  mavlink_msg_raw_imu_pack(sysid, compid, &msg, time_boot_ms, xacc, yacc, zacc, xgyro, ygyro, zgyro, xmag, ymag, zmag);
...
void SendTempPress()
{
  temperature  = 1017.7;    //(Tempsensor.getTemperature(CELSIUS, ADC_512)*10);
...
void SendSystemStatus() {
  ...
  uint16_t volts = ((analogRead(A7) / 319.68 * 5.0) * 1000);
...
void SendAttitude() {
  ...
  int sysid = 1;
  //< The component sending the message.
  int compid = MAV_COMP_ID_IMU_2; //201;
  ...
  //float rollrads = (pitch, 3);  //< Roll angular speed (rad/s)
  //float pitchrads = (roll, 3);  //< Pitch angular speed (rad/s)
  //float yawrads = (yaw, 3);  //< Yaw angular speed (rad/s)
  ...
  mavlink_msg_attitude_pack(sysid, compid, &msg, time_boot_ms, pitch, roll, yaw, rollspeed, pitchspeed, yawspeed);  // purposely switch roll & pitch positions given orientation of sensor within ROV
