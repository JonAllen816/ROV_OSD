...
/* 
 *  
 *  V 2.0 - Roll & Pitch Work!!! - HAD TO REMOVE PRE-PROCESSING CONVERSION OF PITCH AND ROLL FROM RADIANS TO DEGREES AS MINIMOSD IS EXPECTING RADIANS AND CONVERTS ONBOARD
 *  Temp still does not work because of post processing on MINIMOSD to - which is expecting an analog value from 0-1024 and converting Temp to F or C on-board while MS5803 sensor is pre-processing and outputting C & F before send
 *  
 *  TO DO:
 *  CALC MAG TO BE USED AS COMPASS HEADING
 *  CONVERT MS5803 DATA TO ANALOG VALUE OR REMOVE POST PROCESSING SCRIPT FROM MINIMOSD
 *  INTEGRATE / CALIBRATE ANALOG EXTERIOR DEPTH / PRESSURE SENSOR
